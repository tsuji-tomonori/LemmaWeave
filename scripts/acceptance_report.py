...
classified=read(root/'knowledge/declaration-classifications.json')['declarations']
...
for p in (root/'corpus/problems').glob('*.json'):
    ...
    ac={
        ...
        'AC04':{'status':'passed' if problem['status']['semantic']=='independent_checked' else 'inconclusive','reason':'self_review_only; no independent review session exists.'},
        ...
        'AC09':{'status':'passed' if not names-set(classified) else 'inconclusive','unclassified':len(names-set(classified))},
